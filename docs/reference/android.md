...
`setFrameRate(window, rate)` asks for the display to run at `rate` frames a
second for this window (`ANativeWindow_setFrameRate`, API 30+, found at run
time; it returns false where there is none). The system picks the closest
rate the display and its settings allow: on a Pixel with "Smooth Display"
on, `120.0` gives 120 Hz, and with it off the display stays at 60 Hz. The
frame clock follows the display, so an app paced by `postFrameCallback`
draws 120 frames a second only where the request is granted.
